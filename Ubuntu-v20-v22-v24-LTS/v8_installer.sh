...
check_ubuntu_version() {
    ...
    if [[ "$UBUNTU_VERSION" != "20.04" && "$UBUNTU_VERSION" != "22.04" && "$UBUNTU_VERSION" != "24.04" ]]; then
        notification_box "Error: OriginTrail node installer currently requires Ubuntu 20.04, 22.04, or 24.04 LTS versions in order to execute successfully. You are installing on Ubuntu $UBUNTU_VERSION."
...
case "$choice" in
    2) 
        text_color "${GREEN}Fuseki selected. Proceeding with installation."
        tripleStore="ot-fuseki"
        tripleStoreUrl="http://localhost:3030"
        ;;
    [Ee]*) 
        text_color "${RED}Installer stopped by user"
        exit
        ;;
    *) 
        text_color "${GREEN}Blazegraph selected. Proceeding with installation."
        tripleStore="ot-blazegraph"
        tripleStoreUrl="http://localhost:9999"
        ;;
esac
